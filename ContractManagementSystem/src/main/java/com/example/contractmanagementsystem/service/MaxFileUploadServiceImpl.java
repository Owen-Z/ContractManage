...
import com.example.contractmanagementsystem.util.Properties;
...
/**
 * 大文件断点续传
 * @Classname MaxFIleUploadServiceImpl
 * @Date 2020/11/2 14:21
 * @Author pengdz
 */
@Service
@Slf4j
public class MaxFileUploadServiceImpl implements MaxFileUploadService {
    ...
    public JSONObject verifyFileExists(String fileMD5, String fileExt) throws Exception {
        ...
        // 获取合并后的文件
        File uploadFile = new File(properties.getChunkuploadpath().concat(File.separator).concat(fileMD5).concat(".").concat(fileExt));
    ...
    public JSONObject getChunkUploadCount(String fileMD5) throws Exception {
        ...
        // 获取分片存储目录
        File uploadChunkDir = new File(properties.getChunkuploadpath().concat(File.separator).concat(fileMD5));
    ...
    public JSONObject getChunkUploadIndex(String fileMD5, String chunkSize) throws Exception {
        ...
        for(int i=0;i<fileLength;i++){
            ...
            if(files[i].length() != Integer.valueOf(chunkSize))
                files[i].delete();
            else
                chunkIndexArray.add(Integer.valueOf(files[i].getName().split("\\.")[0]));
        }
    ...
    public JSONObject verifyChunk(String fileMD5, String chunk, String chunkSize) throws Exception {
        ...
        try{
            ...
            String uploadPath = properties.getChunkuploadpath().concat(File.separator).concat(fileMD5).concat(File.separator).concat(chunk).concat(".part");
    ...
    public JSONObject upload(MultipartFile file, String fileMD5, String chunk, String chunkSize) throws Exception {
        ...
        try{
            ...
            // 使用MD5字符串作为分片文件存储的目录
            String uploadPath = properties.getChunkuploadpath().concat(File.separator).concat(fileMD5);
     ...
     * @throws Exception
     */
    public JSONObject marginFile(String fileMD5, String fileExt, String chunkCount, String fileName) throws Exception {

        JSONObject resultData = new JSONObject();

        // 获取目标文件输出流
        File outputFile  = new File(properties.getChunkuploadpath().concat(File.separator).concat(fileName));
